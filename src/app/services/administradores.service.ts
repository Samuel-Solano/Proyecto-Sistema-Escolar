...
export class AdministradoresService {
  ...
  public validarAdmin(data: any, editar: boolean) {
    ...
    } else if (data['clave_admin'].length !== 10) {
      error['clave_admin'] =
        'La longitud de caracteres de la clave del administrador no debe ser menor a 10.';
    }
    ...
    if (!editar) {
      if (!this.validatorService.required(data['password'])) {
        ...
        alert('La contraseña minimo debe de tener 8 caracteres.');
      } else if (!this.validatorService.max(data['password'], 10)) {
        error['password'] = this.errorService.max(10);
        alert('La contraseña no debe de superar los 10 caracteres.');
      }
